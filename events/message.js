...
module.exports = {
    run: (message, client) => {
        ...
        if ((message.isMentioned(client.user) || message.channel.type == 'dm' || message.content.endsWith(config.suffix))
            && client.user.id != message.author.id) {
            message.channel.startTyping();
            let cleanMessage = clean(client.user.username, message.cleanContent);
            const user = message.author.id;
            const sessionPath = sessionClient.sessionPath(config.project_id, user);

            let promise = new Promise((resolve, reject) => {
                resolve(sessionClient.detectIntent({
                    session: sessionPath,
                    queryInput: {
                        text: {
                            text: cleanMessage,
